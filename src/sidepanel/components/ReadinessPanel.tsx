...
import { ArrowUpRight, RefreshCcw, Settings2, ShieldCheck, TriangleAlert } from 'lucide-react';
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] ${getStatusTone(report)}`}>
                    {getStatusLabel(report, t)}
                  </span>
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {canOpenModel && (
                    ...
                    >
                      <ArrowUpRight size={13} />
                      <span>
                        {t('readiness.repair.openModel', 'Open model tab')}
                      </span>
                    </button>
                  )}
                  <button
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-[11px]">
                  ...
                  {report.inputReady !== undefined && (
                    <span className="rounded-full border border-slate-200 bg-white px-2.5 py-1 font-medium text-slate-600">
                      {t('readiness.repair.input', 'Input')}: {report.inputReady ? t('readiness.repair.readyShort', 'ready') : t('readiness.repair.missingShort', 'missing')}
                    </span>
                  )}
                  {report.submitReady !== undefined && (
                    <span className="rounded-full border border-slate-200 bg-white px-2.5 py-1 font-medium text-slate-600">
                      {t('readiness.repair.submit', 'Submit')}: {report.submitReady ? t('readiness.repair.readyShort', 'ready') : t('readiness.repair.missingShort', 'missing')}
                    </span>
                  )}
                </div>
              </article>
